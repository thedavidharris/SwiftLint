...
struct LintOptions: OptionsProtocol {
    ...
    static func create(_ path: String) -> (_ useSTDIN: Bool) -> (_ configurationFile: String) -> (_ strict: Bool) -> (_ lenient: Bool) -> (_ forceExclude: Bool) -> (_ useScriptInputFiles: Bool) -> (_ benchmark: Bool) -> (_ reporter: String) -> (_ quiet: Bool) -> (_ cachePath: String) -> (_ ignoreCache: Bool) -> (_ enableAllRules: Bool) -> (_ allowZeroLintableFiles: Bool) -> (_ paths: [String]) -> LintOptions {
        return { useSTDIN in { configurationFile in { strict in { lenient in { forceExclude in { useScriptInputFiles in { benchmark in { reporter in { quiet in { cachePath in { ignoreCache in { enableAllRules in { allowZeroLintableFiles in { paths in
            let allPaths: [String]
            if !path.isEmpty {
                allPaths = [path]
            } else {
    ...
    static func evaluate(_ mode: CommandMode) -> Result<LintOptions, CommandantError<CommandantError<()>>> {
        return create
            ...
            <*> mode <| Option(key: "allow-zero-lintable-files",
                               defaultValue: false,
                               usage: "pass successfully if no files are lintable")
